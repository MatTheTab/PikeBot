...
class PikeBotHeuristic8M(Pikebot):
    ...
    def get_board_score_maia(self, fen: str) -> int:
        '''
        ...
        try:
            info = self.maia_engine.analyse(board, chess.engine.Limit(time=0.1))
        except:
    ...
    def induce_opponents_move(
        ...
        for i in best_probs_index[-self.n_best_moves:]:
            ...
            board.push(next_move)
            self.move_history.append(board.copy())
            score = self.get_board_score_maia(board.fen())
            self.evaluation_history.append(score)
